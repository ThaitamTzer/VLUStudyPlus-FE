...
const schema = v.object({
  student: v.pipe(v.any(), v.nonEmpty('Sinh viên không được để trống')),
    ...
    v.array(
      v.object({
        ...
        termName: v.pipe(
          ...
          v.minLength(3, 'Tên học kỳ phải có ít nhất 3 ký tự'),
  ...
  courseRegistration: v.pipe(
    v.array(
      v.object({
        ...
        note: v.pipe(v.string(), v.maxLength(255, 'Ghi chú không được quá 255 ký tự')),
        termName: v.pipe(
          v.string(),
          v.nonEmpty('Tên học kỳ không được để trống'),
          v.minLength(3, 'Tên học kỳ phải có ít nhất 3 ký tự'),
  ...
  DTBC: v.pipe(
    v.number(),
    v.minValue(1, 'Điểm trung bình không được nhỏ hơn 0'),
    v.maxValue(4, 'Điểm trung bình không được lớn hơn 4')
  ),
  STC: v.pipe(v.number(), v.integer('Số tín chỉ phải là số nguyên'), v.minValue(1, 'Số tín chỉ không được nhỏ hơn 0')),
  DTBCTL: v.pipe(
    v.number(),
    v.minValue(1, 'Điểm trung bình tích lũy không được nhỏ hơn 0'),
    v.maxValue(4, 'Điểm trung bình tích lũy không được lớn hơn 4')
  ),
  STCTL: v.pipe(
    v.number(),
    v.integer('Số tín chỉ tích lũy phải là số nguyên'),
    v.minValue(1, 'Số tín chỉ tích lũy không được nhỏ hơn 0')
  ),
