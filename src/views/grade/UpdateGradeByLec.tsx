...
export default function UpdateGradeByLec({ mutate }: { mutate: KeyedMutator<any> }) {
  const { openUpdateGrade, toogleUpdateGrade, student } = useGradeStore()
  ...
  const { data: trainingPrograms, isLoading: isLoadingTrainingPrograms } = useSWR(
    ['trainingPrograms', pageTrainingProgram, 10, '', '', searchTrainingProgram],
  ...
  return (
    <CustomDialog
      ...
      onClose={handleClose}
      title={`Nhập điểm cho ${student?.userName}`}
      maxWidth='md'
      fullWidth
      onSubmit={onSubmit}
      actions={
        ...
        <Grid item xs={12}>
          <CustomAutocomplete
            ...
            getOptionLabel={option => {
              if (option.cohortId.cohortId === student?.cohortId) {
            ...
            renderOption={(props, option) => (
              <li {...props}>
                {option.title} {option.cohortId.cohortId === student?.cohortId ? ' - Khuyến nghị' : ''}
              </li>
            )}
