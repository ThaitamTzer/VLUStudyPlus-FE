...
export default function AddProcessResult({
  ...
  const onSubmit = handleSubmit(async data => {
    const toastId = toast.loading('Đang thêm kết quả xử lý')

    setLoading(true)

    await resultProcessService.create(
      data,
      () => {
        onClose()
        mutate()
        ...
        toast.update(toastId, {
          render: 'Thêm kết quả xử lý thành công',
          type: 'success',
          isLoading: false,
          autoClose: 3000
        })
  ...
  return (
    <Dialog open={openAddResultProcess} onClose={onClose} maxWidth='sm' fullWidth>
